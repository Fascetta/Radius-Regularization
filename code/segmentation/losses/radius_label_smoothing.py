...
class RadiusLabelSmoothing:
    ...
    def __init__(self, device, n_classes=10, ema_alpha=0.1, manifold="poincare"):
        # EMA of radii per class
        self.device = device
        self.radii_ema = None
        self.max_radii_ema = None
        self.alpha = ema_alpha
        self.n_classes = n_classes
        self.manifold = manifold

    @torch.no_grad()
    def update_ema(self, ema, new):
        if ema is None:
            ema = new
        else:
            ema = self.alpha * new + (1 - self.alpha) * ema
        return ema
    ...
    @torch.no_grad()
    def compute_max_radius_per_class(self, radii, labels):
        max_radius = torch.zeros(self.n_classes, device=self.device)
        for i in range(self.n_classes):
            class_mask = labels == i
            if class_mask.sum() == 0:
                max_radius[i] = self.radii_ema[i]
            else:
                max_radius[i] = radii[class_mask].max()
        return max_radius

    @torch.no_grad()
    def __call__(self, labels, radii):
        radii_per_class = self.compute_mean_radius_per_class(radii, labels)
        self.radii_ema = self.update_ema(self.radii_ema, radii_per_class)

        if self.manifold == "euclidean":
            normalized_radii = self.radii_ema / self.radii_ema.max()
        else:
            normalized_radii = self.radii_ema
        ...
        for i in range(smoothed_labels.size(0)):
            radius_true_class = normalized_radii[labels[i]]
